...
class BookingRepository extends BaseRepository implements IBookingRepository
{
    ...
    public function updateDistanceByJobId(int $jobId, array $data)
    {
        ...
        if ($data['time'] || $data['distance']) {
            $distance = Distance::where('job_id', '=', $jobId)->update(array('distance' => $data['distance'], 'time' => $data['time']));
    ...
    public function updateJobById(int $id, array $data)
    {
        ...
        if ($data["admincomment"] || $data["session"] || $data["flagged"] || $data["manually_handled"] || $data["by_admin"]) {
           $job = Job::where('id', '=', $id)->update(array('admin_comments' => $data["admincomment"], 'flagged' => $data["flagged"], 'session_time' => $data["session"], 'manually_handled' => $data["manually_handled"], 'by_admin' => $data["by_admin"]));
